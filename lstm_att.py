...
class LSTMAtt(nn.Module):
    ...
    def forward(self, target_conv, user_history):
        ...
        his_out = torch.mean(his_out, dim=1).unsqueeze(1)

        hidden_dim = conv_out.size(-1)
        att = conv_out * his_out
        att = F.softmax(att.sum(-1), dim=-1)
        att = att.unsqueeze(-1).repeat(1, 1, hidden_dim)
        model_out = (conv_out * att).sum(1)
